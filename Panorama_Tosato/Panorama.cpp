...
void getClicks(Window w1, Window w2,
    ...
    // ------------- TODO/A completer ----------
    IntPoint2 curPoint;//CONTAIN CURRENT VALUE
    Window curWindow;
    int subWin = 0;
    int finished = false;
    int button;
    ...
    while(!finished)
       {
       button = anyGetMouse(curPoint, curWindow, subWin);
       if(button==3) {
           finished = true;
           return;
       }
       if(curWindow == w1) {
           pts1.push_back(curPoint);
           pointNumber = pts1.size();
           setActiveWindow(w1);
           drawString(curPoint, to_string(pointNumber), MAGENTA);
           drawCircle(curPoint,5,Color(255,0,255),3,false);
           cout << "Point " << pointNumber << " with coordinates " << curPoint << " on window 1!" << endl;
       } else if(curWindow == w2) {
           pts2.push_back(curPoint);
           pointNumber = pts2.size();
           setActiveWindow(w2);
           drawString(curPoint, to_string(pointNumber), MAGENTA);
           drawCircle(curPoint,5,Color(255,0,255),3,false);
           cout << "Point " << pointNumber << " with coordinates " << curPoint << " on window 2!" << endl;
        }
    }
...
Matrix<float> getHomography(const vector<IntPoint2>& pts1,
    ...
    if(n<4) {
        ...
        cout << "Sometimes with 4 points it results in \"cannot solve linear system\" error, even when A has an inverse (checked in R)" << endl;
    }
...
void panorama(const Image<Color,2>& I1, const Image<Color,2>& I2,
    ...
    for (int x = 0; x < I.width(); x++) {
        for (int y = 0; y < I.height(); y++) {
            coord[0] = x+x0; coord[1] = y+y0; coord[2] = 1; // left corner
...
int main(int argc, char* argv[]) {
    ...
    setActiveWindow(w2);
    display(I2,0,0);
    cout << "Please select at least 4 matching points on the two images by clicking on them:"<<endl;
    cout<<"INFO: "<<endl;
    cout<<"LEFT mouse button to select"<<endl;
    cout<<"RIGHT mouse button to terminate and proceed to calculation" << endl;
