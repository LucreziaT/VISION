...
static float correl(const Image<byte>& im1, int i1,int j1,float m1,
    ...
    for (int x=-win; x <= win; x++){
        for (int y=-win; y <= win; y++){
            varim1 += pow(im1(i1+x,j1+y)-m1,2);
            varim2 += pow(im2(i2+x,j2+y)-m2,2);
            rDist += (im1(i1+x,j1+y)-m1)*(im2(i2+x,j2+y)-m2);
        }
    }
    dist = rDist/(sqrt(varim1)*sqrt(varim2));
    return dist;
}
...
static void find_seeds(Image<byte> im1, Image<byte> im2,
                       float nccSeed, // this is the threshold
                       Image<int>& disp, // this is Disparity Map
                       Image<bool>& seeds, // this is Seeds Map (T if correlation > NccSeed )
        ...
        for(int x=win; x+win<im1.width(); x++)
        {
            ...
            for(int xim2 = dMin; xim2 <= dMax + 1; xim2++)
            {
                if( x + xim2 < win ) { continue; }
                    ...
                    if( correlation > bestCorrelation ) // only with best correlation
                    {
                            disparity = xim2;
                            if( disparity < dMin)
                            {
...
while(! Q.empty())
{
        ...
        for(int i = 0; i < 4; i++)
        {
            ...
            if( 0<=x-win && 0<=y-win && x+win<im2.width() && y+win<im2.height() && ! seeds(x,y) )
            {
                ...
                int fi = 1;
                float correlation;
                for (int v = db; v<=fi+1; v++)
                {
                    if( x+v+dispa>win )
                    {
                        correlation = ccorrel(im1, x, y, im2, x+v+dispa, y);
                ...
                seeds(x, y) = true;
                disp(x, y) = BDisparity;
                Q.push(Seed(x, y, BDisparity, bestCorrelation));
...
int main()
{
    ...
    cout<<"Shift + Click on 3D Model to rotate."<<endl;
    show3D(I1,disp);
